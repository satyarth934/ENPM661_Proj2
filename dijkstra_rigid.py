...
##
def getDijkstraPath(input_map, final_map):
	viz_map = copy.deepcopy(final_map)
	...
	while len(minheap) > 0:
		...
		if curr_node.shallowMatch(goal_node):
			...
			for i in path:
				final_map[i.current_coords] = 255
			break
...
def main():
	...
	start_r = int(input("Enter the start row: "))
	start_c = int(input("Enter the start col: "))
	goal_r = int(input("Enter the goal row: "))
	goal_c = int(input("Enter the goal col: "))

	radius = int(input("Enter the robot radius: "))
	clearance = int(input("Enter the clearance: "))

	input_map = obstacles.getMap(radius + clearance, False)
	final_map = obstacles.getMap()
	getDijkstraPath(input_map, final_map)
	cv2.imshow("Map", final_map)
